...
from price_check import magazin_pret_minim
...
class Lista_shop(App):
    def build(self):
        def load_from_file():
            ...
            try:
                ...
                for pr in continut:
                    ...
                    umplutura.text=umplutura.text+'\n'+magazin_pret_minim(pr)


            except Exception as err:
                umplutura.text=f"Unexpected {err=}, {type(err)=}"
        ...
        lista_box=BoxLayout(orientation='vertical',size_hint=(1.0,0.4))
        ...
        umplutura=Label(text='',font_size=marime_font-10,size_hint=(1.0,0.4))
        ...
        def adauga():
            if caseta_text.text!='':
                ...
                umplutura.text = umplutura.text + '\n' + magazin_pret_minim(caseta_text.text)
                caseta_text.text = ''
                salveaza_in_fisier()
